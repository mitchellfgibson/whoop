...
public final class BLEManager: NSObject, ObservableObject {
    ...
    public init(state: LiveState, deviceId: String = "my-whoop") {
        ...
        // Restore identifier + background-capable central. On iOS, the restore identifier is what
        // makes the system RELAUNCH the app in the background to deliver BLE events (connection,
        // notify packets) while the phone is locked/backgrounded — without it `willRestoreState`
        // never fires and the strap can't sync while the phone is "off". macOS has no such mechanism.
    ...
    init(state: LiveState, deviceId: String = "my-whoop", collector: Collector?) {
        ...
        super.init()
        state.lastSyncedAt = UserDefaults.standard.object(forKey: "lastSyncedAt") as? Double
        // iOS: restore identifier enables background relaunch for sync while locked. macOS: plain.
        central = Self.makeCentral(delegate: self)
        // Strap-as-clock: an incoming EVENT packet kicks a rate-limited catch-up sync.
        router.onSyncTrigger = { [weak self] in self?.requestSync(.strap) }
    }

    /// Stable restore identifier — the system uses this to relaunch the app + reattach our central.
    static let restoreIdentifier = "com.noop.ble.central"

    /// Build the central manager. On iOS we pass `CBCentralManagerOptionRestoreIdentifierKey`, which is
    /// the switch that lets the OS WAKE the app in the background to deliver BLE events (so the strap
    /// syncs while the phone is locked / the app isn't foregrounded). macOS doesn't support — and the
    /// restore-identifier option triggers a runtime warning there — so it gets a plain central.
    ...
    private func beginBackfill() -> Bool {
        ...
        beginSyncBackgroundTask()   // keep iOS from suspending us mid-offload (pocket sync)
        backfilling = true
    ...
    // MARK: - Background-task assertion (iOS pocket-sync)

    /// iOS suspends a backgrounded app within seconds of the screen locking — which drops the BLE
    /// link mid-offload, so a chunk never reaches HISTORY_COMPLETE and `lastSyncedAt` never sets (the
    /// "crawls but never completes" symptom). A UIApplication background-task assertion buys ~30 s of
    /// continued execution per offload so chunks can finish committing. Combined with state
    /// restoration (which re-wakes us for the next chunk), the strap can actually sync in your pocket.
    /// No-op on macOS.
    #if os(iOS)
    private var syncBgTask: UIBackgroundTaskIdentifier = .invalid
    #endif

    private func beginSyncBackgroundTask() {
        #if os(iOS)
        guard syncBgTask == .invalid else { return }
        syncBgTask = UIApplication.shared.beginBackgroundTask(withName: "noop.offload") { [weak self] in
            // Expiration handler: iOS is about to reclaim the time — release cleanly so we aren't killed.
    ...
    private func endSyncBackgroundTask() {
        #if os(iOS)
        guard syncBgTask != .invalid else { return }
        UIApplication.shared.endBackgroundTask(syncBgTask)
        syncBgTask = .invalid
        #endif
    }
