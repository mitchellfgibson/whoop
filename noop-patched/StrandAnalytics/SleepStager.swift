...
public enum SleepStager {
    ...
    static func stageSession(start: Int, end: Int, grav: [GravitySample],
        ...
        // GROUND-UP STAGER (v2): physiological emission model + HMM/Viterbi temporal
        // decoding with a sleep-architecture prior, replacing the independent per-epoch
        // percentile classifier. See stageWithHMM. Falls back to legacy if HMM yields nothing.
        var labels = stageWithHMM(feats, onsetIdx: onsetIdx, finalWakeIdx: finalWakeIdx)
        if labels.isEmpty {
            labels = classifyEpochs(feats)
        ...
        // Conservative fragment merge (#274, ported v2.11.0): absorb sub-3-min stage flecks (the
        // WHOOP 5/MG sparse-motion artefact) so the hypnogram stops reading choppier than WHOOP's,
        // WITHOUT erasing genuine multi-minute transitions. Display/scoring smoothing applied AFTER
        // staging — it never touches the HMM per-epoch detection above — and it biases ties toward the
        // LIGHTER stage so it can never inflate deep/REM.
    ...
    // MARK: - Fragment merge (display/scoring smoothing, ported v2.11.0 #274)

    /// Fragment-merge threshold (#274). A staged run shorter than this is "noise": the WHOOP 5/MG
    /// banks sparse motion, so the stager emits sub-minute stage flecks and the hypnogram reads
    /// choppier than WHOOP's. 3 min is conservative — long enough to clear fleck noise, short enough
    /// to leave a genuine stage transition (a real deep/REM block runs many minutes) untouched.
    ...
    /// Sleep-depth rank, lighter → deeper: wake 0, light 1, rem 2, deep 3. Used by mergeFragments to
    /// bias an ambiguous merge toward the LIGHTER stage so smoothing can never inflate deep/REM.
    ...
    /// Display/scoring smoothing of the staged label sequence (#274). Absorbs sub-threshold "noise"
    /// runs WITHOUT erasing real transitions — applied AFTER staging, it never touches the per-epoch
    /// HMM detection. Ties bias toward the lighter neighbour so a stray fleck can never inflate deep/REM.
    static func mergeFragments(_ labels: [String], thresholdEpochs: Int = fragmentMergeEpochs) -> [String] {
        ...
        while i < runs.count {
            ...
            if hasPrev && hasNext && merged[merged.count - 1].stage == runs[i + 1].stage {
                ...
                else { winner = stageDepthRank(prev.stage) <= stageDepthRank(next.stage) ? prev.stage : next.stage }
                ...
                // Leading fleck: fold forward into the next run.
                runs[i + 1] = (stage: runs[i + 1].stage, len: runs[i + 1].len + current.len)
                i += 1
            } else if hasPrev {
                // Trailing fleck: fold back into the previous run.
                merged[merged.count - 1].len += current.len
                i += 1
            } else {
    ...
    // MARK: - Ground-up stager v2: physiological emissions + HMM/Viterbi + architecture prior
    //
    // Replaces the independent per-epoch percentile classifier with a proper temporal
    // model, grounded in the cardiosomnography literature (Walch 2019; HRV staging
    // reviews) and real sleep architecture:
    //   • Deep / slow-wave: lowest HR, highest vagal HRV (RMSSD), low HR variability,
    //     regular respiration — parasympathetic peak.
    //   • REM: elevated HR, HIGH HR variability (sympathetic surges), lower HRV than
    //     deep, irregular respiration, motor atonia (low motion).
    //   • Wake: motion + high HR + high variability.   • Light: intermediate.
    // Architecture: ~90-min cycles, deep concentrated EARLY and decaying, REM minimal
    // in the first ~20 min and GROWING across the night. These are encoded as a
    // time-varying prior + an HMM transition matrix (Viterbi-decoded) so the output is
    // a clean cyclic hypnogram, not noisy per-epoch flicker. Emission weights are
    // calibrated to the user's real WHOOP distribution (~25% deep / 28% REM / 47% light).

    static func stageWithHMM(_ feats: [EpochFeatures], onsetIdx: Int, finalWakeIdx: Int) -> [String] {
        let n = feats.count
        guard n >= 4, finalWakeIdx > onsetIdx, onsetIdx >= 0, finalWakeIdx < n else { return [] }

        // 1. Night-relative z-scores over the sleep period.
        func stats(_ vals: [Double]) -> (Double, Double) {
            let v = vals.filter { $0.isFinite }
            guard !v.isEmpty else { return (0, 1) }
            let m = v.reduce(0, +) / Double(v.count)
            let sd = (v.reduce(0) { $0 + ($1 - m) * ($1 - m) } / Double(v.count)).squareRoot()
            return (m, sd > 1e-6 ? sd : 1)
        }
        let sleepRange = Array(onsetIdx...finalWakeIdx)
        let (hrM, hrSD) = stats(sleepRange.map { feats[$0].hr })
        let (rmssdM, rmssdSD) = stats(sleepRange.map { feats[$0].rmssd })
        let (hrvarM, hrvarSD) = stats(sleepRange.map { feats[$0].hrVar })
        let (rrvM, rrvSD) = stats(sleepRange.compactMap { feats[$0].rrv.isFinite ? feats[$0].rrv : nil })
        func z(_ x: Double, _ m: Double, _ sd: Double) -> Double { x.isFinite ? (x - m) / sd : 0 }

        // 2. Per-epoch emission log-scores: [wake, light, deep, rem].
        var emit = [[Double]](repeating: [Double](repeating: 0, count: 4), count: n)
        for i in 0..<n {
            let f = feats[i]
            let zHR = z(f.hr, hrM, hrSD)
            let zHRV = z(f.rmssd, rmssdM, rmssdSD)   // high → deep
            let zHVar = z(f.hrVar, hrvarM, hrvarSD)  // high → REM/wake
            let zRRV = f.rrv.isFinite ? z(f.rrv, rrvM, rrvSD) : 0  // high → irregular → REM
            let move = f.moveFrac
            let still = move <= stageStillMoveFrac

            // The baselines below are PHYSIOLOGICAL PRIORS, not fit to any small sample:
            // they encode healthy-adult sleep architecture (light/N2 is the majority
            // stage ≈50%, so it is the default ground state with the highest baseline;
            // deep/N3 ≈15-20% and REM ≈20-25% must clear a bar so only a genuine cardiac
            // signature wins). The z-scored signal then drives each epoch — and each
            // NIGHT — away from the prior toward what that night's HR/HRV/motion actually
            // shows. Per-night totals are deliberately NOT forced to a target; they vary.
            emit[i][0] = 2.6 * move + 0.5 * max(0, zHR) + 0.4 * max(0, zHVar) - 1.4   // WAKE (rare mid-sleep, motion-driven)
            emit[i][1] = 1.1 - 0.20 * abs(zHR) - 0.20 * abs(zHVar)                    // LIGHT (majority stage → default)
            emit[i][2] = (-0.85 * zHR) + (0.75 * zHRV) + (-0.6 * zHVar) + (-0.4 * zRRV) // DEEP (low+stable HR, high vagal HRV)
                       + (still ? 0.25 : -1.2)
            emit[i][3] = (0.5 * zHR) + (0.85 * zHVar) + (0.6 * zRRV) + (-0.4 * zHRV)   // REM (HR-variability surges, irregular resp)
                       + (still ? 0.2 : -0.6)
        }

        // 3. Sleep-architecture time-varying prior (deep early-decaying, REM late-growing).
        let onsetClock = feats[onsetIdx].clock
        for i in 0..<n {
            let c = feats[i].clock
            let sinceOnset = max(0, c - onsetClock)
            emit[i][2] += 0.8 - 2.0 * c                          // deep: +early → −late
            let remEarlySuppress = sinceOnset < 0.06 ? -3.0 : 0.0
            emit[i][3] += -0.5 + 1.4 * c + remEarlySuppress      // rem: suppressed early, grows late
        }

        // 4. HMM transition log-probs (realistic sequencing + persistence).
        let stay = 2.2
        func tr(_ from: Int, _ to: Int) -> Double {
        ...
        // 5. Viterbi.
        var dp = [[Double]](repeating: [Double](repeating: -1e9, count: 4), count: n)
        var bp = [[Int]](repeating: [Int](repeating: 0, count: 4), count: n)
        for s in 0..<4 { dp[0][s] = emit[0][s] + (s == 0 ? 1.0 : 0.0) }
        for i in 1..<n {
            for s in 0..<4 {
                var best = -1e9, arg = 0
                for p in 0..<4 {
                    let v = dp[i - 1][p] + tr(p, s)
                    if v > best { best = v; arg = p }
                }
                dp[i][s] = best + emit[i][s]
                bp[i][s] = arg
            }
        }
        var last = 0, bv = -1e9
        for s in 0..<4 where dp[n - 1][s] > bv { bv = dp[n - 1][s]; last = s }
        var path = [Int](repeating: 0, count: n)
        path[n - 1] = last
        for i in stride(from: n - 1, to: 0, by: -1) { path[i - 1] = bp[i][path[i]] }

        let names = ["wake", "light", "deep", "rem"]
        return path.map { names[$0] }
    }
