    ...
    var body: some Scene {
        WindowGroup {
            iOSRootView()
                ...
                // Re-publish the widget as the OFFLOAD progresses, so the lock-screen sync readout
                // advances while the phone is closed (chunks landing + each completed sync).
                .onReceive(model.live.$syncChunksThisSession) { _ in WidgetSnapshot.publish(from: model) }
                .onReceive(model.live.$lastSyncedAt) { _ in WidgetSnapshot.publish(from: model) }
                .onReceive(model.live.$backfilling) { _ in WidgetSnapshot.publish(from: model) }
        }
