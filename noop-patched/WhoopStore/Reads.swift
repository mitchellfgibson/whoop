...
extension WhoopStore {
    ...
    /// Newest timestamp across the OFFLOAD-only streams (gravity + skin temp). Unlike HR — which
    /// streams live over BLE and is therefore always "now" — these only arrive via the historical
    /// offload, so MAX(ts) here is the honest "how far has the deep data actually reached" marker.
    /// A gap between this and now means last night's sleep / sensor data has NOT come down yet,
    /// regardless of how current the live HR is. (#sync-ring)
    public func latestOffloadTs(deviceId: String) async throws -> Int? {
        try syncRead { db in
            let g = try Int.fetchOne(db,
                sql: "SELECT MAX(ts) FROM gravitySample WHERE deviceId = ?", arguments: [deviceId])
            let s = try Int.fetchOne(db,
                sql: "SELECT MAX(ts) FROM skinTempSample WHERE deviceId = ?", arguments: [deviceId])
            return [g, s].compactMap { $0 }.max()
        }
    }
