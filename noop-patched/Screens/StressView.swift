...
struct StressModel {
    ...
    init?(days: [DailyMetric], stored: [(day: String, value: Double)]) {
        ...
        // PATCH: anchor "today" on the most recent day that actually carries a signal — RHR/HRV from
        // the strap, or a stored stress value — NOT just days.last. A freshly-synced strap's literal
        // latest day is often still empty (today's recovery isn't computed until the night completes),
        // which used to make the whole model nil and the screen sit on "Reading…/empty" forever.
        guard let todayIdx = days.lastIndex(where: { d in
            d.restingHr != nil || d.avgHrv != nil || storedByDay[d.day] != nil
        }) else { return nil }
        let today = days[todayIdx]

        // Baseline window: up to 30 days ending the day BEFORE the anchor day, so "today"
        // is measured against its own recent past rather than itself.
        let history = Array(days[..<todayIdx])
