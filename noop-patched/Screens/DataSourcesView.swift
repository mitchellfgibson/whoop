...
struct DataSourcesView: View {
    ...
    /// PATCH (consolidated nav): when embedded (inside Notifications), render the cards
    /// as a section without ScreenScaffold's own scroll/header.
        ...
        Group {
            if embedded {
                VStack(alignment: .leading, spacing: 20) {
                    Text("Data Sources").font(StrandFont.title2).foregroundStyle(StrandPalette.textPrimary)
                    whoopCard
                    appleHealthCard
                    nutritionCard
                    liveCard
