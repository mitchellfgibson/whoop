...
struct LiveView: View {
    ...
    /// PATCH (consolidated nav): when embedded (e.g. at the bottom of Today), render the
    /// content as a section without ScreenScaffold's own scroll/header.
        ...
        Group {
            if embedded {
                VStack(alignment: .leading, spacing: NoopMetrics.sectionGap) {
                    Text("Live").font(StrandFont.title2).foregroundStyle(StrandPalette.textPrimary)
                    liveContent
                }
            } else {
