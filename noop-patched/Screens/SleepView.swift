...
struct SleepView: View {
    ...
    var body: some View {
        ...
        // PATCH: NavigationStack so each metric tile can push that metric's full trend view.
        NavigationStack {
        ...
        // PATCH: tapping a sleep metric tile pushes that metric's full trend/history.
        .navigationDestination(for: MetricDescriptor.self) { MetricDetailView(metric: $0) }
        }
    }
    ...
    private func metricGrid(_ model: SleepModel) -> some View {
        ...
        VStack(alignment: .leading, spacing: NoopMetrics.gap) {
            SectionHeader("Night detail", overline: "Metrics", trailing: "tap to track over time")
            LazyVGrid(columns: tileColumns, alignment: .leading, spacing: NoopMetrics.gap) {

                // PATCH: each tile links to that metric's full trend/history view.
                tappable("sleep_performance", StatTile(
                    label: "Rest",
                ...
                tappable("sleep_efficiency", StatTile(
                    label: "Efficiency",
                ...
                tappable("sleep_consistency", StatTile(
                    label: "Consistency",
                ...
                tappable("hours_vs_needed_pct", StatTile(
                ...
                tappable("restorative_pct", StatTile(
                    label: "Restorative",
                ...
                tappable("sleep_debt_min", StatTile(
                    label: "Sleep Debt",
    ...
    /// PATCH: wrap a sleep StatTile in a NavigationLink to that metric's trend view
    /// (no-op passthrough if the key isn't in the catalog, so it never breaks layout).
    @ViewBuilder
    private func tappable(_ key: String, _ tile: StatTile) -> some View {
        if let descriptor = MetricCatalog.by(key: key) {
            NavigationLink(value: descriptor) { tile }.buttonStyle(.plain)
        } else {
            tile
        }
    }
