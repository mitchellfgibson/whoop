...
struct RootView: View {
    ...
    /// PATCH (consolidated nav): the curated sidebar. Removed entirely (Coach, Breathe,
    /// Intervals, Apple Health, Support). Moved out of the sidebar: Live → bottom of
    /// Today; Data Sources → into Notifications; Health/Trends/Compare → into Explore.
    ...
    var body: some View {
        NavigationSplitView {
            VStack(spacing: 0) {
                List(Self.sidebarItems, selection: $selection) { item in
