...
final class Repository: ObservableObject {
    ...
    func refresh(days nDays: Int = 4000) async {
        ...
        // CSV import, so RHR/HRV/recovery/skin-temp/resp/strain/sleep there were frozen at
        // the last import (March 2025) even while the strap streams fresh data nightly.
    ...
    /// computedDeviceId) so the Explore screen and per-metric trends show fresh strap data.
    private func projectComputedMetricSeries(store: WhoopStore,
                                             computed: [DailyMetric],
                                             computedSleep: [CachedSleepSession]) async {
        var pts: [MetricPoint] = []
        func add(_ day: String, _ key: String, _ v: Double?) {
            if let v, v.isFinite { pts.append(MetricPoint(day: day, key: key, value: v)) }
        }
        for d in computed {
            add(d.day, "rhr", d.restingHr.map(Double.init))
            add(d.day, "hrv", d.avgHrv)
            add(d.day, "recovery", d.recovery)
            add(d.day, "strain", d.strain)
            add(d.day, "spo2", d.spo2Pct)
            add(d.day, "skin_temp", d.skinTempDevC)
            add(d.day, "resp_rate", d.respRateBpm)
            add(d.day, "sleep_total_min", d.totalSleepMin)
            add(d.day, "sleep_efficiency", d.efficiency)
            add(d.day, "sleep_deep_min", d.deepMin)
            add(d.day, "sleep_rem_min", d.remMin)
            add(d.day, "sleep_light_min", d.lightMin)
        }
        guard !pts.isEmpty else { return }
        _ = try? await store.upsertMetricSeries(pts, deviceId: computedDeviceId)
    }
    ...
    func series(key: String, source: String, days: Int = 4000) async -> [(day: String, value: Double)] {
        ...
        // Only the WHOOP source has a computed counterpart; other sources are unaffected.
        ...
        if source == deviceId {
            ...
            for p in computedPts { byDay[p.day] = p.value }   // computed overwrites → recent days live
        }
