...
enum MetricCatalog {
    ...
    /// PATCH: look up a descriptor by key — lets other screens (e.g. Sleep) link a tile
    /// to that metric's full trend/detail view.
    static func by(key: String) -> MetricDescriptor? { all.first { $0.key == key } }
